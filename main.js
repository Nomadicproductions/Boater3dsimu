...
const BREAKING_WAVE_MAX_LENGTH = WATER_SIZE * 0.93; // nearly map-wide
...
// --- SPAWN RIPPLE SET (ALL SAME DIRECTION) ---
function spawnRippleSet() {
  ...
  // base start for first ripple, offset so all ripples are visible on map
  ...
  for (let i = 0; i < setCount; i++) {
    ...
    let start = baseStart.clone().add(
      new THREE.Vector3(
        -dir.y * offset, // perpendicular offset
        0,
        dir.x * offset
      )
    );
    ...
    // Breaking wave: one per ripple, always bound to it
    let bwDuration =
...
function drawBreakingWaves() {
  ...
  for (let i = 0; i < breakingWaves.length; i++) {
    ...
    // Rectangle is centered at the crest, oriented along ripple direction
